...
class BrowserSession:
    def __init__(self, args: argparse.Namespace, root_dir: Path | None = None):
        ...
        self.root_dir = (root_dir or (Path(configured_root) if configured_root else Path.cwd())).resolve()
    ...
    def build_jucewright(self) -> None:
        ...
        if not self.find_jucewright():
            raise StepError("jucewright build completed but the executable could not be resolved")

    def projucer_path(self) -> Path:
        if os.environ.get("PROJUCER_PATH"):
            return Path(os.environ["PROJUCER_PATH"]).expanduser()
